...
from helios.dataset.dataloader_mp4_dist_trans import (
...
def save_control_debug_videos(
    ...
    for sample_idx in range(num_samples):
        ...
        save_path = os.path.join(
            debug_dir,
            f"batch{batch_idx:04d}_sample{sample_idx}_rgb_{control_type}.mp4",
        )

        print(f"🎬 [Debug] Saving RGB-{control_type} compare video: {save_path}")
...
def get_output_path(output_latent_folder, uttid, num_frame, height, width, control_type=None):
    suffix = f"_{control_type}" if control_type else ""
    return os.path.join(
        output_latent_folder,
        f"{uttid}_{num_frame}_{height}_{width}{suffix}.pt",
    )
    ...
    for item in tqdm(dataset.samples):
        try:
            ...
            output_paths = [
                ...
                for control_type in ("edge", "depth")
            ]

            if not all(is_valid_pt(output_path) for output_path in output_paths):
...
def main(
    ...
    print(f"   🔹 sobel_video_folder: {sobel_video_folder}")
    print(f"   🔹 depth_video_folder: {depth_video_folder}")
    ...
    for idx, batch in enumerate(dataloader):
        ...
        if "sobel_videos" not in batch:
            raise KeyError("batch missing `sobel_videos`")

        if "depth_videos" not in batch:
            raise KeyError("batch missing `depth_videos`")
        ...
        for i, (uttid, num_frame, height, width) in enumerate(
            ...
            if append_mode and all(is_valid_pt(output_path) for output_path in output_paths):
                print(f"⏭️ skip existing edge/depth: {uttid}")
                continue
            ...
            valid_sobel_videos.append(batch["sobel_videos"][i])
            valid_depth_videos.append(batch["depth_videos"][i])
        ...
        batch = {
            ...
            "sobel_videos": torch.stack(valid_sobel_videos),
            "depth_videos": torch.stack(valid_depth_videos),
        ...
        with torch.no_grad():
            ...
            control_pixel_values_map = {
                "edge": batch["sobel_videos"].permute(0, 2, 1, 3, 4).to(
                    dtype=vae.dtype,
                    device=device,
                ),
                "depth": batch["depth_videos"].permute(0, 2, 1, 3, 4).to(
                    dtype=vae.dtype,
                    device=device,
                ),
            }
            ...
            for control_type, control_pixel_values in control_pixel_values_map.items():
                print(
                    f"🎮 [Batch {idx}] "
                    f"{control_type}_control_pixel_values: {tuple(control_pixel_values.shape)}"
                )
            ...
            for control_type, control_pixel_values in control_pixel_values_map.items():
                ...
                if control_latents is None:
                    print(f"⚠️ No valid {control_type} control chunks")
                    continue

                control_latents_map[control_type] = control_latents

            if len(control_latents_map) == 0:
                print("⚠️ No valid control chunks")
                continue

            print(f"✅ vae_latents: {tuple(vae_latents.shape)}")
            for control_type, control_latents in control_latents_map.items():
                print(f"✅ {control_type}_control_latents: {tuple(control_latents.shape)}")
        ...
        for control_type, control_latents in control_latents_map.items():
            for (
                ...
                if append_mode and is_valid_pt(output_path):
                    print(f"⏭️ skip existing before save: {uttid} ({control_type})")
                    continue
                ...
                try:
                    ...
                    print(f"✅ save {control_type} latent to: {output_path}")
                except Exception as e:
        ...
        del pixel_values
        del control_pixel_values_map
        del vae_latents
        del control_latents_map
        del prompt_embeds
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--pretrained_model_name_or_path",
        type=str,
        default="/mnt/pfs/users/zhanqian.wu/ckpt/Wan2.1-Fun-V1.1-1.3B-Control-Diffusers",
    )
    ...
    base_video_path = "/shared_disk/users/zhanqian.wu/data/train_data/helios_data/giga_trans_3view"

    video_paths = ["videos"]
    sobel_video_paths = ["sobel_edge_videos"]
    depth_video_paths = ["da2_depth_videos"]

    base_csv_paths = [
        "/shared_disk/users/zhanqian.wu/data/train_data/helios_data/giga_trans_3view",
    ]
    ...
    base_output_latent_path = "/shared_disk/users/zhanqian.wu/data/train_data/helios_data/giga_trans_3view"

    output_latent_paths = [
        "latents_short_giga_trans",
    ]
    ...
    for (
        ...
        sobel_video_folder = os.path.join(base_video_path, sobel_video_path)
        depth_video_folder = os.path.join(base_video_path, depth_video_path)
