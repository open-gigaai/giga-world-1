...
from ...models import GigaworldTransformer3DModel
from ...schedulers import GigaworldScheduler
...
from .modular_pipeline import GigaworldModularPipeline
...
class GigaworldChunkHistorySliceStep(ModularPipelineBlocks):
    ...
    from first chunk by GigaworldChunkUpdateStep).
    """
    ...
    @torch.no_grad()
    def __call__(self, components: GigaworldModularPipeline, block_state: BlockState, k: int):
...
class GigaworldI2VChunkHistorySliceStep(ModularPipelineBlocks):
...
class GigaworldChunkNoiseGenStep(ModularPipelineBlocks):
...
class GigaworldPyramidChunkNoiseGenStep(ModularPipelineBlocks):
    """Generates noise latents and downsamples to smallest pyramid level."""

    model_name = "gigaworld-pyramid"
...
class GigaworldChunkSchedulerResetStep(ModularPipelineBlocks):
    ...
    def expected_components(self) -> list[ComponentSpec]:
        return [
            ComponentSpec("scheduler", GigaworldScheduler),
        ]
...
class GigaworldChunkDenoiseInner(ModularPipelineBlocks):
    ...
    def expected_components(self) -> list[ComponentSpec]:
        return [
            ComponentSpec("transformer", GigaworldTransformer3DModel),
            ComponentSpec("scheduler", GigaworldScheduler),
            ComponentSpec(
                "guider",
...
class GigaworldPyramidChunkDenoiseInner(ModularPipelineBlocks):
    ...
    3. Run timestep denoising loop (same logic as GigaworldChunkDenoiseInner)
    """
    ...
    def __call__(self, components: GigaworldModularPipeline, block_state: BlockState, k: int):
        ...
        # Gigaworld only applies zero init in pyramid stage 0 (lowest resolution), so we disable it
...
class GigaworldChunkUpdateStep(ModularPipelineBlocks):
...
class GigaworldChunkLoopWrapper(LoopSequentialPipelineBlocks):
    """Outer chunk loop that iterates over temporal chunks.

    History indices, scheduler params, and history state are prepared by GigaworldPrepareHistoryStep and
    GigaworldSetTimestepsStep before this block runs. Sub-blocks handle per-chunk preparation, denoising, and history
    updates.
    """
    ...
    @torch.no_grad()
    def __call__(self, components: GigaworldModularPipeline, state: PipelineState) -> PipelineState:
...
class GigaworldChunkDenoiseStep(GigaworldChunkLoopWrapper):
    """T2V chunk-based denoising: history slice -> noise gen -> scheduler reset -> denoise -> update."""

    block_classes = [
        GigaworldChunkHistorySliceStep,
        GigaworldChunkNoiseGenStep,
        GigaworldChunkSchedulerResetStep,
        GigaworldChunkDenoiseInner,
        GigaworldChunkUpdateStep,
    ]
...
class GigaworldI2VChunkDenoiseStep(GigaworldChunkLoopWrapper):
    """I2V chunk-based denoising: I2V history slice -> noise gen -> scheduler reset -> denoise -> update."""

    block_classes = [
        GigaworldI2VChunkHistorySliceStep,
        GigaworldChunkNoiseGenStep,
        GigaworldChunkSchedulerResetStep,
        GigaworldChunkDenoiseInner,
        GigaworldChunkUpdateStep,
...
class GigaworldPyramidDistilledChunkDenoiseInner(ModularPipelineBlocks):
    """Nested pyramid stage loop with DMD denoising for distilled checkpoints.

    Same progressive multi-resolution strategy as GigaworldPyramidChunkDenoiseInner, but:
...
class GigaworldPyramidChunkDenoiseStep(GigaworldChunkLoopWrapper):
    """T2V pyramid chunk denoising: history slice -> pyramid noise gen -> pyramid denoise inner -> update."""

    block_classes = [
        GigaworldChunkHistorySliceStep,
        GigaworldPyramidChunkNoiseGenStep,
        GigaworldPyramidChunkDenoiseInner,
        GigaworldChunkUpdateStep,
    ]
...
class GigaworldPyramidI2VChunkDenoiseStep(GigaworldChunkLoopWrapper):
...
class GigaworldPyramidDistilledChunkDenoiseStep(GigaworldChunkLoopWrapper):
    """T2V distilled pyramid chunk denoising with DMD scheduler and no CFG."""

    block_classes = [
        GigaworldChunkHistorySliceStep,
        GigaworldPyramidChunkNoiseGenStep,
        GigaworldPyramidDistilledChunkDenoiseInner,
        GigaworldChunkUpdateStep,
    ]
    block_names = ["history_slice", "noise_gen", "denoise_inner", "update_chunk"]
...
class GigaworldPyramidDistilledI2VChunkDenoiseStep(GigaworldChunkLoopWrapper):
