...
class GigaworldTextInputStep(ModularPipelineBlocks):
...
class GigaworldAdditionalInputsStep(ModularPipelineBlocks):
...
class GigaworldAddNoiseToImageLatentsStep(ModularPipelineBlocks):
...
class GigaworldAddNoiseToVideoLatentsStep(ModularPipelineBlocks):
...
class GigaworldPrepareHistoryStep(ModularPipelineBlocks):
...
class GigaworldI2VSeedHistoryStep(ModularPipelineBlocks):
    """Seeds history_latents with fake_image_latents for I2V pipelines.

    This small additive step runs after GigaworldPrepareHistoryStep and appends fake_image_latents to the initialized
...
class GigaworldV2VSeedHistoryStep(ModularPipelineBlocks):
    """Seeds history_latents with video_latents for V2V pipelines.

    This step runs after GigaworldPrepareHistoryStep and replaces the tail of history_latents with video_latents. If the
...
class GigaworldSetTimestepsStep(ModularPipelineBlocks):
    ...
    @property
    def description(self) -> str:
        return "Computes scheduler shift parameter (mu) and default sigmas for the Gigaworld chunk loop."
