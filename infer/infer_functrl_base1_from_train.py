...
from diffusers import AutoencoderKLWan, UniPCMultistepScheduler, EulerDiscreteScheduler
...
def main():
    ...
    args.model_config.load_checkpoints_custom = True
    args.model_config.load_model_path = cli_args.checkpoint_path
    ...
    accelerator.print(f"checkpoint           : {cli_args.checkpoint_path}")
    ...
    # prompts = [
    #     "The left robotic arm extends and points at the panda.",
    #     "The left robotic arm stays still, while the right one moves above the panda.",
    #     "The left robotic arm remains stationary. The right arm picks up the panda and places it into the pink bowl.",
    #     "Both robotic arms return to their original positions.",
    # ]

    # interpolate_time_list = [2, 3, 3, 2]
    # interpolation_steps = 1

    pipeline_args = {
        # "prompt": prompts,
        "prompt": "fold the shirt A green T-shirt lies flat on a clean, white table surface. The scene is framed by a dark border at the top and features a partially visible wire mesh or grid structure on the right edge. The lighting is even and bright, casting minimal shadows, and the overall environment is minimalistic with no other objects or clutter visible.",
    ...
    # accelerator.print(f"📝 prompts: {prompts}")
    # accelerator.print(f"📝 interpolate_time_list: {interpolate_time_list}")
    accelerator.print(
    ...
    prefix = os.path.join(
        cli_args.output_dir,
        f"{cli_args.sample_name}_{ts}",
    )

    gen_path = f"{prefix}_gen.mp4"
    control_save_path = f"{prefix}_control.mp4"
    first_frame_path = f"{prefix}_first_frame.jpg"

    export_to_video(output, gen_path, fps=cli_args.fps)
    export_to_video(control_video, control_save_path, fps=cli_args.fps)
    image.save(first_frame_path)

    accelerator.print(f"✅ saved gen       : {gen_path}")
    accelerator.print(f"✅ saved control   : {control_save_path}")
    accelerator.print(f"✅ saved first img : {first_frame_path}")
...
if __name__ == "__main__":
    ...
    sys.argv = [
        sys.argv[0],
        "--config", "/mnt/pfs/users/zhanqian.wu/code/Gigaworld/scripts/training/configs/stage_1_post_functrl.yaml",
        "--base_model_path", "/mnt/pfs/users/zhanqian.wu/ckpt/Wan2.1-Fun-V1.1-1.3B-giga-ctrl-2200",
        "--transformer_model_name_or_path","/mnt/pfs/users/zhanqian.wu/ckpt/stage-3-init/stage1_final_3v_uni_s16k",
        "--checkpoint_path", "/mnt/pfs/users/zhanqian.wu/ckpt/stage-3-init/stage1_final_3v_uni_s16k/",
        "--control_video_path", "None",
        "--image_path", "None",
        "--prompt", "None",
        "--output_dir", "/mnt/pfs/users/zhanqian.wu/output/infer_like_train",
        "--sample_name", "functrl_like_train",
        "--num_frames", "660",
        "--height", "480",
        "--width", "1920",
    ...
    base_folder = "/shared_disk/users/zhanqian.wu/data/infer_data/helios_data/giga_ctrl_3view/fold_the_shirt_easy"
    ...
    if video_pairs:
        for idx, pair in enumerate(video_pairs):
            ...
            img = get_first_frame_image(pair["video_path"], 1920, 480)
            if img is None:
                print(f"⚠️ 读取首帧失败，跳过: {pair['video_path']}")
                continue

            temp_img = f"./infer/temp_gt_first_frame_{idx}.jpg"
            os.makedirs(os.path.dirname(temp_img), exist_ok=True)
            img.save(temp_img)

            for i in range(len(sys.argv)):
                if sys.argv[i] == "--control_video_path" and i + 1 < len(sys.argv):
                    sys.argv[i + 1] = pair["control_video_path"]
                if sys.argv[i] == "--image_path" and i + 1 < len(sys.argv):
                    sys.argv[i + 1] = temp_img
                if sys.argv[i] == "--sample_name" and i + 1 < len(sys.argv):
                    sys.argv[i + 1] = f"functrl_like_train_{idx + 1:03d}"

            print("🎯 推理参数:")
            print(f"   视频: {pair['video_path']}")
            print(f"   控制视频: {pair['control_video_path']}")
            print(f"   输出样本名: functrl_like_train_{idx + 1:03d}")
            print("✅ 使用多段机械臂动作提示词")
