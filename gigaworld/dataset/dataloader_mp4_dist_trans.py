...
def read_cut_crop_and_resize(
    ...
    sobel_video_data = _read_one_video_cut_crop_resize(
        video_path=sobel_video_path,
        f_prime=f_prime,
        h_prime=h_prime,
        w_prime=w_prime,
        stride=stride,
    ...
    depth_video_data = _read_one_video_cut_crop_resize(
    ...
    return video_data, sobel_video_data, depth_video_data
...
class BucketedFeatureDataset(Dataset):
    def __init__(
        ...
        sobel_video_folders=None,
        depth_video_folders=None,
        stride=1,
        base_fps=None,
        resolution=640,
        force_rebuild=True,
        ...
        if sobel_video_folders is None:
            self.sobel_video_folders = self.control_video_folders
        else:
            self.sobel_video_folders = (
                [sobel_video_folders]
                if isinstance(sobel_video_folders, str)
                else sobel_video_folders
            )

        if depth_video_folders is None:
            self.depth_video_folders = self.control_video_folders
        else:
            self.depth_video_folders = (
                [depth_video_folders]
                if isinstance(depth_video_folders, str)
                else depth_video_folders
            )

        assert len(self.json_files) == len(self.video_folders)
        assert len(self.json_files) == len(self.control_video_folders)
        assert len(self.json_files) == len(self.sobel_video_folders)
        assert len(self.json_files) == len(self.depth_video_folders)

        self.samples = []
        self.buckets = defaultdict(list)

        for json_file, video_folder, sobel_video_folder, depth_video_folder in zip(
            self.json_files,
            self.video_folders,
            self.sobel_video_folders,
            self.depth_video_folders,
            ...
            self._process_json_file(
                ...
                sobel_video_folder=sobel_video_folder,
                depth_video_folder=depth_video_folder,
    ...
    def _process_json_file(
        ...
        if self.force_rebuild or not os.path.exists(cache_file):
            ...
            print(f"  Sobel video folder: {sobel_video_folder}")
            print(f"  Depth video folder: {depth_video_folder}")
    ...
    def _build_file_metadata(
        ...
        try:
            with open(json_file, "r", encoding="utf-8") as f:
                data = json.load(f)
            print(f"✅ 加载标准 JSON 文件: {json_file}")
        except:
            data = []
            bad_lines = 0
            with open(json_file, "r", encoding="utf-8") as f:
                for line_no, line in enumerate(f, 1):
                    ...
                    try:
                        ...
                        bad_lines += 1
                        if bad_lines <= 10:
                            print(f"⚠️ skip bad jsonl line {line_no}: {line[:120]}")
            if bad_lines > 0:
                print(f"⚠️ skipped bad jsonl lines: {bad_lines}")
            print(f"✅ 加载 JSONL 文件: {json_file}")
        # ====================================================================
        ...
        print(f"Scanning sobel video folder: {sobel_video_folder}")
        existing_sobel_videos = set()

        for root, dirs, files in os.walk(sobel_video_folder):
            for file in files:
                if file.endswith(".mp4"):
                    rel_path = os.path.relpath(os.path.join(root, file), sobel_video_folder)
                    existing_sobel_videos.add(rel_path)

        print(f"Found {len(existing_sobel_videos)} sobel video files")

        print(f"Scanning depth video folder: {depth_video_folder}")
        existing_depth_videos = set()

        for root, dirs, files in os.walk(depth_video_folder):
            for file in files:
                if file.endswith(".mp4"):
                    rel_path = os.path.relpath(os.path.join(root, file), depth_video_folder)
                    existing_depth_videos.add(rel_path)

        print(f"Found {len(existing_depth_videos)} depth video files")

        rows = []

        for item in data:
            if "sobel_edge_path" not in item:
                raise KeyError(
                    "json item missing `sobel_edge_path`. "
                    "Please add sobel_edge_path for each sample."
                )

            if "da2_depth_path" not in item:
                raise KeyError(
                    "json item missing `da2_depth_path`. "
                    "Please add da2_depth_path for each sample."
                )
            ...
            rows.append(
                {
                    ...
                    "crop": item["crop"],
                    "path": item["path"],
                    "sobel_edge_path": item["sobel_edge_path"],
                    "da2_depth_path": item["da2_depth_path"],
        ...
        for i, row in df.iterrows():
            ...
            sobel_video_file = self._normalize_video_rel_path(row["sobel_edge_path"])

            if sobel_video_file not in existing_sobel_videos:
                print(f"bad sobel video: {sobel_video_file}")
                continue

            sobel_video_path = os.path.join(sobel_video_folder, sobel_video_file)

            depth_video_file = self._normalize_video_rel_path(row["da2_depth_path"])

            if depth_video_file not in existing_depth_videos:
                print(f"bad depth video: {depth_video_file}")
                continue

            depth_video_path = os.path.join(depth_video_folder, depth_video_file)
            ...
            sample_info = {
                ...
                "depth_video_folder": depth_video_folder,
                "video_path": video_path,
                "sobel_video_path": sobel_video_path,
    ...
    def __getitem__(self, idx):
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                video_data, sobel_video_data, depth_video_data = read_cut_crop_and_resize(
                    video_path=sample_info["video_path"],
                    sobel_video_path=sample_info["sobel_video_path"],
                    depth_video_path=sample_info["depth_video_path"],
                ...
                return {
                    ...
                    "videos": video_data,
                    "sobel_videos": sobel_video_data,
                    "depth_videos": depth_video_data,
                    "control_videos": sobel_video_data,
                ...
                print(f"Error sobel {sample_info.get('sobel_video_path')}")
                print(f"Error depth {sample_info.get('depth_video_path')}")
                idx = random.randint(0, len(self.samples) - 1)
                retry_count += 1
...
if __name__ == "__main__":
    ...
    # ============================================================
    json_file = [
        "/shared_disk/users/zhanqian.wu/data/train_data/helios_data/giga_trans_3view/helios_giga_ctrl.jsonl",
    ]

    video_folder = [
        "/shared_disk/users/zhanqian.wu/data/train_data/helios_data/giga_trans_3view/videos",
    ]

    sobel_video_folder = [
        "/shared_disk/users/zhanqian.wu/data/train_data/helios_data/giga_trans_3view/sobel_edge_videos",
    ]

    depth_video_folder = [
        "/shared_disk/users/zhanqian.wu/data/train_data/helios_data/giga_trans_3view/da2_depth_videos",
    ]
    ...
    dataset = BucketedFeatureDataset(
        ...
        sobel_video_folders=sobel_video_folder,
        depth_video_folders=depth_video_folder,
        stride=stride,
        force_rebuild=True,
    ...
    assert "sobel_videos" in sample, "❌ missing sobel_videos"
    assert "depth_videos" in sample, "❌ missing depth_videos"
    assert "first_frames_images" in sample, "❌ missing first_frames_images"

    video = sample["videos"]
    sobel_video = sample["sobel_videos"]
    depth_video = sample["depth_videos"]
    ...
    print(f"sobel_videos shape: {tuple(sobel_video.shape)}")
    print(f"depth_videos shape: {tuple(depth_video.shape)}")
    print(f"video range: {video.min().item():.4f}, {video.max().item():.4f}")
    print(f"sobel range: {sobel_video.min().item():.4f}, {sobel_video.max().item():.4f}")
    print(f"depth range: {depth_video.min().item():.4f}, {depth_video.max().item():.4f}")

    assert isinstance(video, torch.Tensor)
    assert isinstance(sobel_video, torch.Tensor)
    assert isinstance(depth_video, torch.Tensor)
    assert video.shape == sobel_video.shape == depth_video.shape, (
        f"❌ video/sobel/depth shape mismatch: "
        f"{tuple(video.shape)} vs {tuple(sobel_video.shape)} vs {tuple(depth_video.shape)}"
    )
    assert video.ndim == 4, f"❌ videos should be [T,C,H,W], got {video.shape}"
    assert sobel_video.ndim == 4, f"❌ sobel_videos should be [T,C,H,W], got {sobel_video.shape}"
    assert depth_video.ndim == 4, f"❌ depth_videos should be [T,C,H,W], got {depth_video.shape}"
    assert video.shape[1] == 3, f"❌ video channel should be 3, got {video.shape[1]}"
    assert sobel_video.shape[1] == 3, f"❌ sobel channel should be 3, got {sobel_video.shape[1]}"
    assert depth_video.shape[1] == 3, f"❌ depth channel should be 3, got {depth_video.shape[1]}"
    ...
    for step, batch in enumerate(dataloader):
        ...
        assert "sobel_videos" in batch, "❌ batch missing sobel_videos"
        assert "depth_videos" in batch, "❌ batch missing depth_videos"
        assert "prompts" in batch, "❌ batch missing prompts"
        assert "first_frames_images" in batch, "❌ batch missing first_frames_images"

        videos = batch["videos"]
        sobel_videos = batch["sobel_videos"]
        depth_videos = batch["depth_videos"]
        ...
        print(f"sobel_videos: {tuple(sobel_videos.shape)}")
        print(f"depth_videos: {tuple(depth_videos.shape)}")
        print(f"first_frames_images: {tuple(batch['first_frames_images'].shape)}")
        print(f"prompt[0]: {batch['prompts'][0]}")

        assert videos.shape == sobel_videos.shape == depth_videos.shape, (
            f"❌ batch video/sobel/depth mismatch: "
            f"{tuple(videos.shape)} vs {tuple(sobel_videos.shape)} vs {tuple(depth_videos.shape)}"
        )

        assert videos.ndim == 5, f"❌ videos should be [B,T,C,H,W], got {videos.shape}"
        assert sobel_videos.ndim == 5, f"❌ sobel_videos should be [B,T,C,H,W], got {sobel_videos.shape}"
        assert depth_videos.ndim == 5, f"❌ depth_videos should be [B,T,C,H,W], got {depth_videos.shape}"

        b, t, c, h, w = videos.shape
        assert c == 3, f"❌ video C should be 3, got {c}"
        assert sobel_videos.shape[2] == 3, f"❌ sobel C should be 3, got {sobel_videos.shape[2]}"
        assert depth_videos.shape[2] == 3, f"❌ depth C should be 3, got {depth_videos.shape[2]}"
        ...
        if save_debug and step < 3:
            rgb = videos[0]
            sobel = sobel_videos[0]
            depth = depth_videos[0]

            # [T,C,H,W] -> concat width
            vis = torch.cat([rgb, sobel, depth], dim=-1)

            save_path = os.path.join(save_dir, f"debug_batch{step:03d}_rgb_sobel_depth.mp4")
