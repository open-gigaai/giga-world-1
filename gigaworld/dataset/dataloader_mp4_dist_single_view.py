...
class BucketedFeatureDataset(Dataset):
    ...
    def _build_file_metadata(self, json_file, video_folder, control_video_folder):
        ...
        #             vid_count += 1
        #             print(f"\rMain video scanned: {vid_count}", end="", flush=True)
        # print(f"\nFound {len(existing_videos)} video files\n")
        ...
        #             ctrl_count += 1
        #             print(f"\rControl video scanned: {ctrl_count}", end="", flush=True)
        # print(f"\nFound {len(existing_control_videos)} control video files")
        ...
        for item in data:
            ...
            if item["crop"] == [0, 1920, 0, 480]:
                num_view = 3
                item["crop"] = [0, 1920//num_view, 0, 480]
                print (f"[info ⚠️] changing crop to {item['crop']}")
        ...
        for i, row in df.iterrows():
            ...
            if not os.path.exists(control_video_path):
                print(f"bad control video: {control_video_path}")
                continue
