...
def _flow_loss(
    ...
    for noisy_model_input, sigmas, timesteps, target in zip(
        ...
        # loss = loss * (batch_size / total_sample_count) NOTE: 0616 ZKEY FIX
        if os.environ.get("GIGAWORLD_FORCE_ZERO_GRAD_FOR_UNUSED_PARAMS", "0") == "1":
            zero_anchor = None
            for param in transformer.parameters():
                if param.requires_grad:
                    anchor = param.sum() * 0.0
                    zero_anchor = anchor if zero_anchor is None else zero_anchor + anchor
            if zero_anchor is not None:
                loss = loss + zero_anchor
...
def _flow_loss_ctrl(
    ...
    assert control_target_latents is not None, "control_target_latents is None"
    assert control_target_latents.ndim == 5, (
        f"control_target_latents should be [B,C,T,H,W], got {control_target_latents.shape}"
    )
    ...
    for noisy_model_input, sigmas, timesteps, target in zip(
        ...
        # control 对齐 noisy input
        control = control_target_latents.to(
            device=noisy_model_input.device,
            dtype=noisy_model_input.dtype,
            non_blocking=True,
        )

        assert control.shape[0] == noisy_model_input.shape[0], (
            f"control batch mismatch: {control.shape} vs {noisy_model_input.shape}"
        )
        assert control.shape[2:] == noisy_model_input.shape[2:], (
            f"control spatial/temporal mismatch: {control.shape} vs {noisy_model_input.shape}"
        )
        ...
        if isinstance(model_pred, list):
            ...
            for cur_model_pred, cur_target, cur_sigmas in zip(model_pred, target, sigmas):
                ...
                loss = torch.mean(
                    (
                        cur_weighting.float()
                        * (cur_model_pred.float() - cur_target.float()) ** 2
                    ).reshape(cur_target.shape[0], -1),
                    1,
                ).mean()
