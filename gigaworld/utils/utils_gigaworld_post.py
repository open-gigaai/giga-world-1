...
from .utils_gigaworld_base import (
...
# ======================================== ODE Loss ========================================
# ============================================================
# Stage1-only ODE regression loss
# ============================================================
# ============================================================
# Small helpers copied/simplified for standalone ODE regression
# ============================================================
def generate_and_sync_flag(accelerator, num_steps: int, last_step_only: bool, is_sync: bool = False) -> int:
    if last_step_only:
        idx = num_steps - 1
    else:
        idx = random.randint(0, num_steps - 1)

    if is_sync and accelerator.num_processes > 1:
        t = torch.tensor(idx, device=accelerator.device, dtype=torch.long)
        torch.distributed.broadcast(t, src=0)
        idx = int(t.item())
    return idx


def _ode_regression_loss_stage1_functrl(
    args,
    accelerator,
    transformer,
    scheduler,
    ...
    """
    Stage1-only ODE regression for FunCtrl.

    ODE format per section:
      ode_latents[k][0]["latents"]         [K+1, B, 16, T, H, W]  scheduler state
      ode_latents[k][0]["model_input"]     [K+1, B, 48, T, H, W]  FunCtrl transformer input
      ode_latents[k][0]["noise_pred"]      [K,   B, 16, T, H, W]  teacher flow
      ode_latents[k][0]["timesteps"]       [K]

    Loss follows original Gigaworld ODE regression style:
      student flow -> pred_x0, compare pred_x0 to teacher final x0.
    """
    assert ode_latents is not None, "ode_latents is required"
    assert ode_prompt_embeds is not None, "ode_prompt_embeds is required"
    assert is_keep_x0, "stage1 ODE regression currently assumes is_keep_x0=True"
    assert len(ode_latents) > 0
    assert len(ode_latents[0]) == 1, f"Stage1-only expects one stage, got {len(ode_latents[0])}"

    # Use ODE data shape, not noise channel shape, because FunCtrl noise may be 48ch while history is 16ch.
    first_stage = ode_latents[0][0]
    first_latent = first_stage["latents"][0]
    batch_size, scheduler_channels, latent_window_size, height, width = first_latent.shape
    ...
    total_sections = len(ode_latents)
    ode_num_sections = sample_dynamic_num_sections(
        min_sections=ode_num_latent_sections_min,
        max_sections=ode_num_latent_sections_max,
        dynamic_alpha=ode_dynamic_alpha,
        dynamic_beta=ode_dynamic_beta,
        sample_type=ode_dynamic_sample_type,
        global_step=global_step,
        dynamic_step=ode_dynamic_step,
        total_sections=total_sections,
    )
    selected_sections = sorted(random.sample(range(total_sections), ode_num_sections))
    ...
    scheduler_sigmas = scheduler.sigmas.to(accelerator.device) if hasattr(scheduler, "sigmas") else None
    ...
    for k in range(total_sections):
        ...
        # ------------------------------------------------------------
        # Build AR history exactly like original stage1 path
        # ------------------------------------------------------------
        if is_first_section:
            ...
            indices = torch.arange(
                0,
                sum([1, *history_sizes, latent_window_size]),
                device=accelerator.device,
            )
            (
                indices_prefix,
            ...
            del history_latents_first_section
        else:
            indices = torch.arange(
                0,
                sum([1, *history_sizes, latent_window_size]),
        ...
        assert len(ode_latents[k]) == 1, f"Stage1-only expects one stage at section {k}, got {len(ode_latents[k])}"
        ode_stage = ode_latents[k][0]

        # Teacher x0 for this section. This is always 16ch scheduler latent.
        gt_x0 = ode_stage["latents"][-1].to(accelerator.device, dtype=weight_dtype, non_blocking=True)

        if should_compute_grad:
            num_ode_steps = int(ode_stage["timesteps"].shape[0])
            step_idx = generate_and_sync_flag(accelerator, num_ode_steps, last_step_only, is_sync=False)

            # Transformer input: FunCtrl uses 48ch model_input. Original fallback uses 16ch latents.
            input_key = "model_input" if "model_input" in ode_stage else "latents"
            noisy_model_input = ode_stage[input_key][step_idx].to(
                accelerator.device, dtype=weight_dtype, non_blocking=True
            )
            # Scheduler xt must always be 16ch. Do NOT use 48ch model_input here.
            scheduler_xt = ode_stage["latents"][step_idx].to(
                accelerator.device, dtype=weight_dtype, non_blocking=True
            )

            timestep = ode_stage["timesteps"][step_idx].view(1).to(
                accelerator.device, dtype=torch.long, non_blocking=True
            )

            # Note: for stage1-only standalone training we keep schedule shift as identity by default.
            # If you need exact original dynamic shifting, plug apply_schedule_shift here.
            sigmas_for_x0 = scheduler_sigmas
            timesteps_for_x0 = scheduler_timesteps
            ...
            model_pred = transformer(
                ...
                latents_history_short=latents_history_short.to(device=accelerator.device, dtype=ode_prompt_embeds.dtype),
                latents_history_mid=latents_history_mid.to(device=accelerator.device, dtype=ode_prompt_embeds.dtype),
                latents_history_long=latents_history_long.to(device=accelerator.device, dtype=ode_prompt_embeds.dtype),
                return_dict=False,
            )[0]
            ...
            pred_x0 = flow_to_x0(
                ...
                xt=scheduler_xt,
                timestep=timestep,
                sigmas=sigmas_for_x0,
                timesteps=timesteps_for_x0,
            )

            loss_i = 0.5 * F.mse_loss(pred_x0.float(), gt_x0.float(), reduction="mean")
            ode_loss_list.append(loss_i)

            del noisy_model_input, scheduler_xt, timestep, model_pred, pred_x0, loss_i

        # ------------------------------------------------------------
        # Update teacher-forced history with gt_x0
        # ------------------------------------------------------------
        if is_first_section:
        ...
        del gt_x0
        del indices, indices_prefix, indices_latents_history_long, indices_latents_history_mid
    ...
    if len(ode_loss_list) == 0:
        raise RuntimeError("ode_loss_list is empty. Check ode_num_latent_sections_* settings.")

    ode_loss = torch.stack(ode_loss_list).mean() * ode_regression_weight
    logs = {
        "ode_loss": ode_loss.detach().item(),
        "ode_num_sections": ode_num_sections,
    }
...
def _ode_regression_loss(
    ...
    ode_control_latents: torch.Tensor = None,
    ...
    first_item = ode_latents[0]
    # 兼容旧格式
    if isinstance(first_item, (list, tuple)):
        first_item = first_item[0]

    batch_size, _, _, _, _ = first_item["latents"][0].shape
    ...
    for k in range(total_ode_num_latent_sections):
        ...
        if should_compute_grad:
            ode_item = ode_latents[k]

            # 兼容旧保存格式：ode_latents[k][0]
            if isinstance(ode_item, (list, tuple)):
                ode_item = ode_item[0]

            exit_flag = generate_and_sync_flag(
                accelerator,
                ode_item["timesteps"].shape[0],
                last_step_only,
                is_sync=False,
            )

            # 48ch: FunCtrl transformer input
            noisy_model_input = ode_item["model_input"][exit_flag].to(
                accelerator.device,
                dtype=weight_dtype,
            )

            # 16ch: scheduler xt，用于 flow -> x0
            scheduler_xt = ode_item["latents"][exit_flag].to(
                accelerator.device,
                dtype=weight_dtype,
            )

            gt_x0 = ode_item["latents"][-1].to(
                accelerator.device,
                dtype=weight_dtype,
            )

            timestep = ode_item["timesteps"][exit_flag].unsqueeze(0).to(
                accelerator.device,
            )

            # timesteps_per_stage = scheduler.timesteps.to(
            #         accelerator.device
            # )
            timesteps_per_stage = ode_item["timesteps"].to(
                accelerator.device,
                dtype=torch.float32,
            )

            if "sigmas" in ode_item:
                #print(f"✅ using sigmas from ODE data for section {k}")
                sigmas_per_stage = ode_item["sigmas"].to(
                    accelerator.device,
                    dtype=torch.float32,
                )
            else:
                # 兼容旧 ODE 数据；新数据应该直接有 sigmas
                sigmas_per_stage = timesteps_per_stage / 1000.0
                # sigmas_per_stage = scheduler.sigmas.to(
                #     accelerator.device
                # )

            # =====================================================
            # DEBUG: 确认 timestep / sigma 来自 ODE 文件
            # =====================================================
            # if global_step < 30 and k == 0:
            #     print("\n" + "=" * 80)
            #     print("ODE DEBUG FROM SAVED PT")
            ...
            #     print(
            #         "ode timesteps:",
            #         timesteps_per_stage.detach().cpu().tolist(),
            #     )

            #     print(
            #         "ode sigmas:",
            #         sigmas_per_stage.detach().cpu().tolist(),
            #     )

            #     nearest_idx = torch.argmin(
            #         torch.abs(
            #             timesteps_per_stage.to(timestep.device)
            #             - timestep.flatten()[0]
            #         )
            #     )

            #     print("nearest idx:", nearest_idx.item())
            #     print("nearest timestep:", timesteps_per_stage[nearest_idx].item())
            #     print("nearest sigma:", sigmas_per_stage[nearest_idx].item())

            #     print(
            #         "scheduler_xt std:",
            #         scheduler_xt.float().std().item(),
            #     )

            #     print(
            #         "gt_x0 std:",
            #         gt_x0.float().std().item(),
            #     )
            ...
            #     print(
            #         "latents shape:",
            #         tuple(ode_item["latents"].shape),
            #     )

            #     print(
            #         "timesteps shape:",
            #         tuple(ode_item["timesteps"].shape),
            #     )

            #     print(
            #         "sigmas shape:",
            #         tuple(sigmas_per_stage.shape),
            #     )
            ...
            pred_x0 = convert_flow_pred_to_x0(
                flow_pred=model_pred,
                xt=scheduler_xt,  # 16ch，不要用 48ch noisy_model_input
                timestep=timestep,
                sigmas=sigmas_per_stage,
                timesteps=timesteps_per_stage,
            )

            temp_mse_loss = 0.5 * F.mse_loss(
                pred_x0.float(),
                gt_x0.float(),
                reduction="mean",
            )
            ...
            del scheduler_xt
            del timestep
            del timesteps_per_stage
            del sigmas_per_stage
            del model_pred
            del pred_x0
            del temp_mse_loss
...
def build_functrl_input(scheduler_xt, gt_control_latents, section_idx, model_type):
    # 调试打印：输入信息
    # print(f"\n===== build_functrl_input 调试信息 =====")
    # print(f"section_idx: {section_idx}")
    # print(f"scheduler_xt shape: {scheduler_xt.shape}, dtype: {scheduler_xt.dtype}, device: {scheduler_xt.device}")
    if gt_control_latents is None:
        print("gt_control_latents is None, 直接返回 scheduler_xt")
        return scheduler_xt

    # print(f"gt_control_latents shape: {gt_control_latents.shape}, dtype: {gt_control_latents.dtype}, device: {gt_control_latents.device}")

    latent_window_size = scheduler_xt.shape[2]
    start = section_idx * latent_window_size
    end = start + latent_window_size
    #print(f"latent_window_size: {latent_window_size}, start: {start}, end: {end}")

    if gt_control_latents.shape[2] == latent_window_size:
        control = gt_control_latents
        #print(f"gt_control长度与窗口一致，直接使用")
    else:
        assert gt_control_latents.shape[2] >= end, (
            f"control length not enough: "
            f"{gt_control_latents.shape=}, section_idx={section_idx}, "
            f"need [{start}:{end}]"
        )
        control = gt_control_latents[:, :, start:end]
        # print(f"截取 gt_control 片段: [{start}:{end}], 截取后 shape: {control.shape}")

    # 类型/设备对齐
    control = control.to(device=scheduler_xt.device, dtype=scheduler_xt.dtype)
    # print(f"control 对齐后 dtype: {control.dtype}, device: {control.device}")

    # 维度校验
    if model_type in ("wan2.1", "wan2.2"):
        assert scheduler_xt.shape[1] == 16, f"scheduler_xt 通道数错误: {scheduler_xt.shape}"
        assert control.shape[1] == 16, f"control 通道数错误: {control.shape}"
        assert control.shape[2:] == scheduler_xt.shape[2:], (
            f"control 与 xt 空间维度不匹配: {control.shape=} vs {scheduler_xt.shape=}"
        )
        output = torch.cat(
            [scheduler_xt, control, torch.zeros_like(scheduler_xt)],
            dim=1,
        )
    else:
        assert scheduler_xt.shape[1] == 48, f"scheduler_xt 通道数错误: {scheduler_xt.shape}"
        assert control.shape[1] == 48, f"control 通道数错误: {control.shape}"
        mask_latents = torch.zeros(
            scheduler_xt.shape[0],
            4,
            scheduler_xt.shape[2],
            scheduler_xt.shape[3],
            scheduler_xt.shape[4],
            device=scheduler_xt.device,
            dtype=scheduler_xt.dtype,
        )

        # 拼接
        output = torch.cat(
            [scheduler_xt, control, mask_latents, torch.zeros_like(scheduler_xt)],
            dim=1,
        )
...
def inference_with_trajectory_stage1(
    ...
    # For Control
    gt_control_latents: torch.Tensor = None,
    # type
    model_type: str = "wan2.1", # NOTE: 0608 ADD HERE 
):

    #raise NotImplementedError  FIXME: DO NOT USE IN ORIGINAL GIGAWORLD, BUT USED IN GIGA
    ...
    for k in range(num_rollout_sections):
        ...
        for index, current_timestep in enumerate(denoising_step_list):
            ...
            if not exit_flag:
                with torch.no_grad():
                    model_input_48ch = build_functrl_input(noisy_model_input, gt_control_latents, k, model_type)
                    model_pred = transformer(
                        hidden_states=model_input_48ch, # NOTE: ORI noisy_model_input
                        timestep=timestep,
                        encoder_hidden_states=prompt_embeds,
                        indices_hidden_states=indices_hidden_states,
                        indices_latents_history_short=indices_latents_history_short,
    ...
    return (
        output,                    # generated rollout latent [B,C,T,H,W]
        denoised_timestep_from,    # rollout start timestep
        denoised_timestep_to,      # rollout end timestep
        consistency_align_loss,    # x0 consistency regularization
    )


def inference_with_trajectory_pyramid(
    args,
    accelerator,
    transformer,
    scheduler,
    ...
    pyramid_num_inference_steps_list: list = [20, 20, 20],
    # For DMD Main
    denoising_step_list: list = None,
    last_step_only: bool = False,
    last_section_grad_only: bool = False,
    ...
    for i_s in range(pyramid_num_stages):
        num_denoising_steps = pyramid_num_inference_steps_list[i_s]
    ...
    if is_multi_pyramid_stage_backward_simulated:
        divisor = 2 ** (pyramid_num_stages - 1 - init_pyramid_stage_flag)
            ...
            if is_amplify_first_chunk and is_first_section:
                if not is_use_gt_history:
                    scheduler.set_timesteps(
                        pyramid_num_inference_steps_list[i_s] * 2 + 1, i_s, device=accelerator.device
                    )
                elif (
                    ...
                    scheduler.set_timesteps(pyramid_num_inference_steps_list[i_s] + 1, i_s, device=accelerator.device)
            else:
                scheduler.set_timesteps(pyramid_num_inference_steps_list[i_s] + 1, i_s, device=accelerator.device)
            ...
            if is_amplify_first_chunk and is_first_section:
                if not is_use_gt_history:
                    init_exit_flag = generate_and_sync_flag(
                        accelerator, pyramid_num_inference_steps_list[i_s] * 2, last_step_only
                    )
                elif (
                    latents_history_short.sum() == 0
                    and latents_history_mid.sum() == 0
                    ...
                    init_exit_flag = generate_and_sync_flag(
                        accelerator, pyramid_num_inference_steps_list[i_s] * 2, last_step_only, is_sync=False
                    )
                else:
    ...
    if is_multi_pyramid_stage_backward_simulated:
        ...
        scheduler.set_timesteps(
            pyramid_num_inference_steps_list[init_pyramid_stage_flag] + 1,
...
def consistency_backward_simulation(
    ...
    history_sizes: list = [16, 2, 1],
    # Pyramid Rollout
    use_pyramid_rollout: bool = False,
    pyramid_num_stages: int = 3,
    pyramid_num_inference_steps_list: list = [20, 20, 20],
    # For DMD Main
    denoising_step_list: list = None,
    ...
    common_kwargs = {
        ...
        # type
        "model_type": model_type, # NOTE: 0608 ADD HERE 
    }

    if use_pyramid_rollout:
        pyramid_kwargs = {
        ...
        return inference_with_trajectory_pyramid(**common_kwargs, **pyramid_kwargs)
    else:
        stage1_kwargs = {
...
def run_generator(
    ...
    gt_control_latents: torch.Tensor = None,
    # type
    model_type: str = None, # NOTE: 0608 ADD HERE 
    ...
    pred_image_or_video, denoised_timestep_from, denoised_timestep_to, consistency_align_loss = (
        consistency_backward_simulation(
            ...
            history_sizes=history_sizes,
            # For Pyramid Rollout
            use_pyramid_rollout=use_pyramid_rollout,
            pyramid_num_stages=pyramid_num_stages,
            pyramid_num_inference_steps_list=pyramid_num_inference_steps_list,
            # For DMD Main
            denoising_step_list=denoising_step_list,
...
def build_functrl_score_input(scheduler_xt, control_latents,model_type):
    """
    scheduler_xt:     [B,16,T,H,W]
    control_latents:  [B,16,T,H,W] 或 [B,16,total_T,H,W]
    return:           [B,48,T,H,W]
    """
    ...
    if control_latents.shape[2] == latent_window_size:
        control = control_latents
    else:
        control = control_latents[:, :, -latent_window_size:]

    control = control.to(device=scheduler_xt.device, dtype=scheduler_xt.dtype)

    if model_type in ["wan2.1", "wan2.2"]:
        assert scheduler_xt.shape[1] == 16, scheduler_xt.shape
        assert control.shape[1] == 16, control.shape
        assert control.shape[2:] == scheduler_xt.shape[2:], (
            f"{control.shape=} {scheduler_xt.shape=}"
        )
        zero_latent = torch.zeros_like(scheduler_xt)
        output = torch.cat([scheduler_xt, control, zero_latent], dim=1)
    elif model_type == "wan2.2_5b":
        assert scheduler_xt.shape[1] == 48, scheduler_xt.shape
        assert control.shape[1] == 48, control.shape
        assert control.shape[2:] == scheduler_xt.shape[2:], (
            f"{control.shape=} {scheduler_xt.shape=}"
        )
        zero_latent = torch.zeros_like(scheduler_xt)
        ...
        output = torch.cat(
            [scheduler_xt, control, mask_latents, zero_latent],
            dim=1,
        )
...
def compute_kl_grad(
    ...
    noisy_image_or_video,           # 加noise后视频
    estimated_clean_image_or_video, # 模型预测的x0, 真实视频 latent
    prompt_embeds,
    ...
    # print(f"is_decouple_dmd mode: {is_decouple_dmd}")
    # ==================================================
    # Step 1: Fake Prediction
    #
    # enable_adapters():
    #     Teacher + LoRA
    #
    ...
    pred_fake_image_cond = real_fake_score_model(
        ...
        hidden_states=build_functrl_score_input(noisy_image_or_video,gt_control_latents,model_type),
...
def compute_distribution_matching_loss(
    accelerator,
    scheduler,
    real_fake_score_model,      # frozen score/teacher model for DMD gradient estimation
    image_or_video,             # Student generated latent [B,C,T,H,W]
    ...
    with torch.no_grad():
        # Randomly sample a timestep and re-noise Student output
        ...
        if is_decouple_dmd:
            ...
            ca_timestep = sample_dynamic_timestep(
                ...
                min_step=min_step,
                max_step=max_step,
                timestep_shift=timestep_shift if not use_pyramid_rollout and timestep_shift > 1 else 1.0,
                dynamic_alpha=dynamic_alpha,
                dynamic_beta=dynamic_beta,
                dynamic_sample_type=dynamic_sample_type,
                global_step=global_step,
...
def _generator_loss(
    ...
    pyramid_num_inference_steps_list: list = None,
    # For DMD Main
    denoising_step_list: list = None,
    last_step_only: bool = False,
    last_section_grad_only: bool = False,
    ...
    if is_multi_pyramid_stage_backward_simulated:
        ...
        init_pyramid_stage_flag = random.randint(0, pyramid_num_stages - 1)
    ...
    # print(
    #     "\n🧪🧪🧪 [DMD-GEN OUTPUT] Student rollout done",
    #     f"\n  🧱 pred_image_or_video: shape={tuple(pred_image_or_video.shape)}",
    #     f"mean={pred_image_or_video.float().mean().item():.6f}",
    #     f"std={pred_image_or_video.float().std().item():.6f}",
    #     f"min={pred_image_or_video.float().min().item():.6f}",
    #     f"max={pred_image_or_video.float().max().item():.6f}",
    #     f"\n  ⏱ denoise_from={denoised_timestep_from}",
    #     f"denoise_to={denoised_timestep_to}",
    #     f"\n  🧩 grad_mask={gradient_mask is not None}",
    #     flush=True,
    # )
...
def _critic_loss(
    ...
    if is_use_gan: #TODO: STILL UNDER CONSTRUCTION...
        raise NotImplementedError("GAN loss is still under construction, stay tuned!")
        ...
        #         hidden_states=build_functrl_score_input(noisy_fake_latent,gt_control_latents ),
        ...
        #     hidden_states_list = [ #TODO: NOTE: For ControlNets, we need to build the input with control latents
        #         build_functrl_score_input(noisy_fake_latent, gt_control_latents),
        #         build_functrl_score_input(noisy_real_latent, gt_control_latents),
        #     ]
        ...
        flow_fake_pred = fake_score_model(
            # hidden_states=noisy_fake_latent,
            hidden_states=build_functrl_score_input(noisy_fake_latent,gt_control_latents,model_type),
            timestep=critic_timestep,
            encoder_hidden_states=prompt_embeds,
            indices_hidden_states=indices_hidden_states,
            indices_latents_history_short=indices_latents_history_short,
