...
from ..modules.transformer_functrl_gigaworld import GigaworldTransformer3DModelFunCtrl
from ..scheduler.scheduling_gigaworld import GigaworldScheduler
from ..utils.utils_base import AdaptiveAntiDrifting, apply_schedule_shift
from ..utils.utils_gigaworld_post import add_noise, convert_flow_pred_to_x0
from .pipeline_output import GigaworldPipelineOutput
...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        >>> from diffusers import AutoencoderKLWan, GigaworldPipeline

        >>> # Available models: BestWishYsh/Gigaworld-Base, BestWishYsh/Gigaworld-Mid, BestWishYsh/Gigaworld-Distilled
        >>> model_id = "BestWishYsh/Gigaworld-Base"
        >>> vae = AutoencoderKLWan.from_pretrained(model_id, subfolder="vae", torch_dtype=torch.float32)
        >>> pipe = GigaworldPipeline.from_pretrained(model_id, vae=vae, torch_dtype=torch.bfloat16)
        >>> pipe.to("cuda")
...
class GigaworldFunCtrlPipeline(DiffusionPipeline, WanLoraLoaderMixin):
    r"""
    Pipeline for text-to-video / image-to-video / video-to-video generation using Gigaworld.
    ...
    Args:
        ...
        transformer ([`GigaworldTransformer3DModelFunCtrl`]):
    ...
    def __init__(
        ...
        vae: AutoencoderKLWan,
        scheduler: UniPCMultistepScheduler | GigaworldScheduler,
        transformer: GigaworldTransformer3DModelFunCtrl,
        pipeline_type: str = "wan2.1",
    ):
        super().__init__()
    ...
    def prepare_control_video_latents(
        self,
        ...
        if latents is None:
            ...
            for i in range(num_chunks):
                chunk_start = start_frame + i * min_frames
                chunk_end = chunk_start + min_frames
                video_chunk = control_video[:, :, chunk_start:chunk_end, :, :]
                chunk_latents = self.vae.encode(video_chunk).latent_dist.sample(generator=generator)
                chunk_latents = (chunk_latents - latents_mean) * latents_std
                latents_chunks.append(chunk_latents)
            latents = torch.cat(latents_chunks, dim=0) # NOTE: FOR CONTROL SIGNAL, CONCAT IN BATCH DIM TO FEED INTO TRANSFORMER
        return first_frame_latent.to(device=device, dtype=dtype), latents.to(device=device, dtype=dtype)
    ...
    def stage1_sample(
        ...
        for i, t in enumerate(timesteps):
            ...
            if self.pipeline_type == "wan2.2_5b":
                assert latents.shape[1] == 48, f"scheduler latents must be 48, got {latents.shape}"
            else:
                assert latents.shape[1] == 16, f"scheduler latents must be 16, got {latents.shape}"
            assert control_latents is not None, "FunCtrl pipeline requires control_latents"

            cur_control_latents = control_latents.to(device=device, dtype=transformer_dtype)

            if cur_control_latents.ndim == 5:
                pass
            elif cur_control_latents.ndim == 6:
                cur_control_latents = cur_control_latents[:, 0]
            else:
                raise ValueError(f"Bad control_latents shape: {control_latents.shape}")

            cur_control_latents = cur_control_latents[:, :, : latents.shape[2]]

            if self.pipeline_type == "wan2.2_5b":
                latent_model_input = torch.cat(
                    [
                        latents.to(device=device, dtype=transformer_dtype),
                        cur_control_latents,
                        torch.zeros(
                            latents.shape[0],
                            4,
                        ...
                        torch.zeros_like(latents, device=device, dtype=transformer_dtype),
                    ],
                    dim=1,
                )
                assert latent_model_input.shape[1] == 148
            elif self.pipeline_type == "wan2.1":
                latent_model_input = torch.cat(
                    [
                        latents.to(transformer_dtype),
                        cur_control_latents,
                        torch.zeros_like(latents).to(transformer_dtype),
                    ],
                    dim=1,
                )
                assert latent_model_input.shape[1] == 48
            else:
                raise ValueError(f"Bad model_type: {self.model_type}")
    ...
    def __call__(
        ...
        # ------------ CONTROL ------------
        control_video   : Optional[PipelineImageInput] = None,
        control_latents: Optional[torch.Tensor] = None,
        # ------------ Interactive ------------
        ...
        Args:
            ...
            return_dict (`bool`, *optional*, defaults to `True`):
                Whether or not to return a [`GigaworldPipelineOutput`] instead of a plain tuple.
        ...
        Returns:
            [`~GigaworldPipelineOutput`] or `tuple`:
                If `return_dict` is `True`, [`GigaworldPipelineOutput`] is returned, otherwise a `tuple` is returned where
        ...
        if control_video is not None:
            ...
            control_image_latents, control_latents = self.prepare_control_video_latents(
                control_video,
                latents_mean=latents_mean,
                latents_std=latents_std,
                num_latent_frames_per_chunk=9, # TODO: HARDCODE REMOVE LATER
                dtype=torch.float32,
                device=device,
                generator=generator,
                latents=video_latents,
        ...
        num_channels_latents = 48 if self.pipeline_type == "wan2.2_5b" else 16
        ...
        for k in range(num_latent_sections):
            control_latents_cur = None
            if control_latents is not None:
                control_latents_cur = control_latents[k:k+1].to(device=device, dtype=transformer_dtype)
            ...
            with self.progress_bar(total=num_inference_steps) as progress_bar:
                latents = self.stage1_sample(
                    latents=latents,
                    control_latents=control_latents_cur,
                    prompt_embeds=prompt_embeds,
                    negative_prompt_embeds=negative_prompt_embeds,
                    timesteps=timesteps,
        ...
        return GigaworldPipelineOutput(frames=video)
