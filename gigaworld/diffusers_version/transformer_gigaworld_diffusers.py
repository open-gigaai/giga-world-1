...
class GigaworldAttnProcessor:
    ...
    def __call__(
        ...
        hidden_states = attn.to_out[1](hidden_states) # FIXME: This is a temporary fix for a performance regression caused by the addition of the output dropout layer in GigaworldAttention. We should investigate and optimize this further.     
        ...
        # print(
        #     f"QKV={(t1-t0)*1000:.1f}ms "
        #     f"NormRoPE={(t3-t2)*1000:.1f}ms "
        #     f"Attn={(t4-t3)*1000:.1f}ms "
        #     f"OutProj={(t5-t4)*1000:.1f}ms"
        # )
    ...
    def __init__(
        ...
        if has_multi_term_memory_patch:
            self.patch_short = nn.Conv3d(
                16, #in_channels, TODO: 16 FIXED FOR WAN 2.1
                self.inner_dim, 
